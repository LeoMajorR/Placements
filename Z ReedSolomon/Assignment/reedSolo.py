...
def find_prime_polys(generator=2, c_exp=8, fast_primes=False, single=False):
    ''' Find all primitive polynomials for GF(2**c_exp)'''
    root_charac = 2
...
def gf_add(x, y):
    ''' Add two elements of GF(2**m)'''
    return x ^ y


def gf_sub(x, y):
    ''' Subtract two elements of GF(2**m)'''
    return x ^ y


def gf_neg(x):
    ''' Additive inverse of an element of GF(2**m)'''
    return x


def gf_inverse(x):
    ''' Multiplicative inverse of an element of GF(2**m)'''
...
def gf_pow(x, power):
    ''' Raise an element of GF(2**m) to a power'''
    return gf_exp[(gf_log[x] * power) % field_charac]


def gf_mult_noLUT_slow(x, y, prim=0):
    ''' Multiplication in GF(2**m) without LUTs '''
...
def rs_generator_poly(nsym, fcr=0, generator=2):
    ''' Generator polynomial for RS code'''
    g = _bytearray([1])
...
def rs_generator_poly_all(max_nsym, fcr=0, generator=2):
    ''' Generator polynomials for all RS codes'''
    g_all = [[1]] * max_nsym
...
def rs_simple_encode_msg(msg_in, nsym, fcr=0, generator=2, gen=None):
    ''' Encode a message using RS code'''
    global field_charac
...
def rs_calc_syndromes(msg, nsym, fcr=0, generator=2):
    ''' Calculate syndromes for a message'''
    return [0] + [
...
def rs_find_errata_locator(e_pos, generator=2):
    ''' Find errata locator polynomial'''
    e_loc = [1]
    for i in e_pos:
        e_loc = gf_poly_mul(
...
def rs_find_errors(err_loc, nmess, generator=2):
    ''' Find error positions from the error locator polynomial'''
    err_pos = []
...
def rs_forney_syndromes(synd, pos, nmess, generator=2):
    ''' Forney algorithm for syndromes'''
...
class RSCodec(object):
    ...
    def decode(self, data, nsym=None, erase_pos=None, only_erasures=False):
        '''Decode data with Reed-Solomon ECC'''
        global gf_log, gf_exp, field_charac
        gf_log, gf_exp, field_charac = self.gf_log, self.gf_exp, self.field_charac
    ...
    def check(self, data, nsym=None):
        '''Check data with Reed-Solomon ECC'''
        if not nsym:
            nsym = self.nsym
        if isinstance(data, str):
            data = _bytearray(data)
